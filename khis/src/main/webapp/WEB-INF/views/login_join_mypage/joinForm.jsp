	...
	<div class="container">
		<div class="joinForm">
			...
			<div class="memberId-container">
				<h4>아이디</h4>
				<input type="text" id="id" name="id" value="" placeholder="아이디를 입력하세요" required><br />
			...
			<div class="content" id="companyNo">
				<h4>사업자등록번호</h4>
<!-- 			<input type="number"  id="tmpCoCode"> -->
				<input type="number" name="coCode" id="coCode" readonly>
...
<!--  			<div class="content" id="companyNo2">
				<h4>사업자등록번호</h4>
				<input type="number" name="coCode" id="coCode2">
			</div>  --> 
			...
			<div class="content" id="email">
				<h4>이메일</h4>
				<input type="email" name="email" id="email" placeholder="ex)asdf@naver.com" required><br />
			</div>

			<div class="content">
				<h4>휴대전화</h4>
				<input type="tel"  name="phone" id="phone" placeholder="(-없이 입력)" required><br />
			</div>

			<div class="content">
				<h4>주소</h4>
				<input type="text" name="address" id="address" placeholder="주소를 입력하세요" required><br />
			</div>
			...
			<br>
			<div class="aggrement">
				<input type="checkbox" class="aggrement" id="aggrement" onClick="a()">
				<label class="test" for="test">개인정보 수집 및 이용에 동의합니다.</label>
			</div>
			<br> <br>
...
$(document).ready(function() {
  $('#kind').change(function() {
    ...
    if (['IR_SUPERVISOR'].includes(result)) {
        window.open(`${pageContext.request.contextPath}/member/irSKind.do`, "a","width=500, height=660, left =100, top=100"); 
        $('#companyNo').show();
    	}
      $('#coName').hide();
      $('input#search').hide();
...
/* if(document.getElementById("kind").value == "USER"){
...
/* //가입자 유형 (면접자,면접관: 면접번호 입력창 o  / 면접관리자,예비면접자: 면접번호 입력창 x)
$(document).ready(function() {
  $('#kind').change(function() {
   // var result = $('#kind option:selected').val();
    var result = this.value;
    console.log('selected value:', result);
    if (['IR_SUPERVISOR', 'IR'].includes(result)) {
      $('#companyNumber').show();
    } else {
      $('#companyNumber').hide();
    }
